...
contract ExposedMerchItem is MerchItem{
  ...
  function hasPurchasedItem() public returns(bool) {
    return _hasPurchasedItem();
  }

  function calculateTotalPayment(uint256 numOfItem) public returns (uint256) {
    return _calculateTotalPayment(numOfItem);
  }

  function calculatePriceOfItem(uint256 itemNum) public returns (uint256) {
  ...
  function mappingPatronToList() public returns(bool) {
    // create a new Patron struct and push it to patrons(mapping)
    return _mappingPatronToList();
    
  }

  function calculatePortionOfFunds() public returns(bool) {
  ...
  function transferFundsToCompound() public returns(bool) {
    ...
    function specialDiv(uint256 a, uint256 b, uint256 precision) public view returns (uint256) {
...
contract TestMerchItem {
  ...
  function testHasPurchasedItem() public {
